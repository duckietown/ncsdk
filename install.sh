    ...
    if [ "${OS_DISTRO,,}" = "ubuntu" ] && [ ${OS_VERSION} = 1604 ]; then
       # Require 64-bit Ubuntu OS
        ...
        if [ "${HARDWARE_PLATFORM}" != "x86_64" ] ; then
            echo -e "${RED} You are not running on a 64-bit OS version which is required."
            #exit 1
        fi
...
function install_python_dependencies()
{
	...
	exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET  numpy==1.15"
        ...
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET  scikit-image>=0.13.0,<=0.14.0"
        #NPS exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET  --upgrade numpy"
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET  pygraphviz Enum34>=1.1.6 numpy==1.15 networkx>=2.1,<=2.1"
        # Install packages for python 2.x, required for NCSDK python API
        exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET  Enum34>=1.1.6"
        #NPS exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET  --upgrade numpy"
	exec_and_search_errors "$PIP_PREFIX pip2 install $PIP_QUIET  numpy==1.15"
...
function install_tensorflow()
{
    ...
    if [[ $tf -ne 0 && $tf_gpu -ne 0 ]]; then
        ...
        exec_and_search_errors "$PIP_PREFIX pip3 install $PIP_QUIET  --trusted-host www.piwheels.org tensorflow==$SUPPORTED_TENSORFLOW_VERSION"
    else
